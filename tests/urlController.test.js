...
describe('Shorten URL Endpoint', () => {
  it('should shorten a URL and return the shortened URL', async () => {
    const response = await request(app)
      ...
      .send({ originalUrl: 'https://www.w3schools.com/python/trypython.asp?filename=demo_class_str1' })
      .expect(201);

    expect(response.body).toHaveProperty('status', 'success');
    expect(response.body).toHaveProperty('data');
    expect(response.body.data).toHaveProperty('originalUrl', 'https://www.w3schools.com/python/trypython.asp?filename=demo_class_str1' );
